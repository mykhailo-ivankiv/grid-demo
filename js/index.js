/**
 * Init part of whole system
 * @author Mykhailo Ivankiv [neformal.lviv@gmail.com]
 */

define(['GridSystem', 'ToolBox', 'WidgetContainer'],
  function (GridSystem,  ToolBox,  WidgetContainer) {
    "use strict";
    ...
    widgetContainer
      ...
      .bind("changeWidget", function (widget) {gridSystem.getWidgetHolder(widget).html(widget.html); })
      .bind("addWidget", function (widget) {
        gridSystem.getWidgetHolder(widget).html(widget.html);

        gridSystem.getWidgetHolder(widget).addClass("widget")
          .data("widget", widget)
          .draggable({
            cursorAt: { top: -35, left: -35},
            helper: function () {return $.tmpl(gridSystem.cellTemplate, widget.name); }
          });
      })
      .bind("saveWidget", function (widget) {gridSystem.getWidgetHolder(widget).html(widget.html); })
      .bind("editWidget", function (widget) {gridSystem.getWidgetHolder(widget).html(widget.html); })
      .bind("selectWidget", function (widget) {gridSystem.getWidgetHolder(widget).addClass('active'); })
      .bind("unselectWidget", function (widget) {gridSystem.getWidgetHolder(widget).removeClass("active"); });
