...
define(['Widget', "EventMachine", 'WidgetContainer', 'GridSystemConfig'],
  function (Widget,  EventMachine,  WidgetContainer,   GridSystemConfig) {
    "use strict";
    var GridSystem = function () {
      ...
      function getElementByPos(x, y, selector) {
        ...
        for (i = 0; i < elements.length; i += 1) {
          ...
          // Check the element
          if (y > elementOffset.top && y <= (elementOffset.top  + elementOffset.height) &&
              x > elementOffset.left && x <= (elementOffset.left + elementOffset.width)) {
            return element;
          }
        }
        return null;
      ...
      function getWidget(x, y) { return getElementByPos(x, y, this_.wClassSelector); }
      function getCell(x, y) { return getElementByPos(x, y, this_.cClassSelector); }
      function getRow(x, y) { return getElementByPos(x, y, this_.rClassSelector); }
      ...
      function getRelatedPosition(x, y, settings) {
        settings = settings ||
          { cellManipulation: true, h: 25, /* horizontalRegion (%) */ v: 25 /* verticalRegion (%) */ };
        ...
        if (row) {
          ...
          if (relativeY < settings.h || relativeY > (100 - settings.h)) { //top is default direction
      ...
      function getWidgetRelatedPosition(x, y) { //check only vertical position
        ...
        return {element: widget, row: cell.parent(), cell: cell, direction : direction};
      }
      ...
      function getHelperHTML(extendObj) {
        ...
        helper.css({width: width, top: top, bottom: bottom, left: left, height: height});
        return helper;
      }

      function highlightObj(e) {
        if (e.ctrlKey || e.metaKey) { //TODO: bind to window keydown
      ...
      function getLayoutArray(row, from, to) {
        ...
        for (i = (from || 0); i < (to || cells.length); i += 1) {
          cells[i].className.replace(SELECT_GRID, function () { layout.push(parseInt(arguments[1])); })
        }
        return layout;
      }
      ...
      function getExtendedCell(extend) {
        ...
        if (extend.direction === "left") {
          ...
          } else {cell.insertAfter(cells.last()); } // insert in the end of row
      ...
      function dropElement_(event, ui) {
        ...
        } else if (widget && extend.direction === "center") { //replace
          alert("Replace still not implemented");
        } else if (widget && extend.direction !== "center") { //relocate
      ...
      this.getWidgetHolder = function (widget) { return $("[widgetid=" + widget.id + "]"); };
      this.getWidgetCell = function (widget) { return this_.getWidgetHolder(widget).parent(); };

      function getWidgetRow(widget) { return this_.getWidgetCell(widget).parent(); }
        ...
        function resizeAction(currentEvent) {
          ...
          if (prevWidth !== deltaColumn) {
            ...
            if (currentCellWidth >= this_.wMinWidth) {
              ...
              if (layout) {
                this_.resizableCell[0].className = this_.resizableCell[0].className.replace(SELECT_GRID, " grid_" + currentCellWidth + " ");
      ...
      resizeLeft = $("<div class='resize-left'></div>").bind("mousedown", function (e) { resize(e, 'left'); });
      resizeRight = $("<div class='resize-right'></div>").bind("mousedown", function (e) { resize(e, 'right'); });
      ...
      this.relocateWidget = function (widget, newCell) {
        ...
        if (!oldCell.find(this_.wClassSelector)[0]) { oldCell.remove(); } // Cell is empty;
        if (!oldRow.find(this_.cClassSelector)[0]) {                      // Row is empty;
          oldRow.remove();
        } else {
      ...
      this.addWidget = function (widgetRootConfig, cell) {
        ...
        } else { widgetHolder.appendTo(cell); }
      ...
      this.getLayout = function () {
        var layout = this_.$container.clone().
          find(this_.wClassSelector)
          .each(function () {$(this).removeClass("active").html(""); })
          .end();
        return layout.html();
      };
      ...
      function init_() {
        ...
        this_.$container
          .droppable({
            over: extend_,
            out : function () {highlightEl_.css({display: "none"}); },
          ...
          .delegate(this_.wClassSelector, "mousedown", function () { widgetContainer.selectWidget($(this).data("widget")); })
          .delegate(this_.wClassSelector, "dblclick", function () { widgetContainer.editWidget($(this).data("widget")); });
        ...
        $(window).bind('keypress', function (e) {
          if ((e.keyCode === 46 || e.keyCode === 8) && widgetContainer.activeWidget.mode !== "edit") {  //Delete (or Backspace)
            widgetContainer.removeWidget(widgetContainer.activeWidget);
            e.preventDefault();
          }
          if (e.keyCode === 13 && widgetContainer.activeWidget.mode !== "edit") {  //Enter
