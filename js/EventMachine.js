...
define(function () {
  ...
  return function () {
    ...
    this.one = function (ev, fn) { //TODO: rewrite to unbind;
      ev = ev.toLocaleLowerCase();
      ev = (ev.indexOf('on') !== 0) ? 'on' + ev : ev;
    ...
    this.trigger = function (ev, arg) {
      ...
      ev = ev.toLocaleLowerCase();
      ev = (ev.indexOf('on') !== 0) ? 'on' + ev : ev;
      for (i = 0; events[ev] && i < events[ev].length; i += 1) {events[ev][i].apply(this, arg); }
      for (i = 0; eventsOne[ev] && i < eventsOne[ev].length; i += 1) {eventsOne[ev][i].apply(this, arg); }
      delete eventsOne[ev];
