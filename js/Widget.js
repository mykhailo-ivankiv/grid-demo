define(['EventMachine'], function (EventMachine) {
  ...
  var Widget = function (rootConfig, id) {
     ...
     */
    this.clone = function () { //TODO: check uses
      var newObj = function () {var this_ = this; };
    ...
    this.getSettings = function () {
      return getter_(this_.src + this.settingsSrc, 'json')
   ...
   */
    ...
    this.previewMode = function(mode, settings) {
      return $.when(getModeTemplate_(mode), getModeBehaviour_(mode))
       .pipe(function (template, modeFun) {
    ...
    var getModeHTML_ = function(mode){
      return $.when (getModeTemplate_(mode),this_.getSettings())
              .pipe(function(template, settings){
                      var html = $.tmpl(template, settings);
                      return html;
                    });
    };
   ...
   */
    this.activateMode = function(mode){
      ...
      return loadMode_(mode)
        .done(function(html,modeFun){
          this_.html = html;
          execMode_(modeFun,modeFun.modules);
    ...
    this.changeCategory = function(rootConfig, settings){
      ...
      for (var tmp in rootConfig){this_[tmp] = rootConfig[tmp]};
    ...
    function init_ (){
      this_.rootConfig = rootConfig;
      for (var tmp in rootConfig){this_[tmp] = rootConfig[tmp]}; //TODO: Refactor;
    }
